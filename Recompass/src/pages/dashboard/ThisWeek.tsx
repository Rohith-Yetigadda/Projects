...
type SavedMenu = { uploadedAt: string; extractedData: MealPlan[]; };
...
export default function ThisWeek() {
  ...
  if (!menu) return (
    <div className="max-w-lg mx-auto flex flex-col items-center py-24 gap-6 text-center">
      ...
      <Link to="/app/menu-upload" className="inline-flex items-center gap-2 px-8 py-4 bg-white text-black font-bold rounded-2xl hover:scale-[1.02] transition-transform">
        Upload menu <ArrowRight className="w-4 h-4" />
      </Link>
    </div>
  );

  const sortedMenu = [...menu.extractedData].sort((a, b) => DAYS.indexOf(a.day) - DAYS.indexOf(b.day));
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-28 md:pb-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {DAYS.map((day, i) => {
          ...
          let cls = "bg-white/5 text-white/50 hover:bg-white/10 hover:text-white";
          if (isSelected) cls = "bg-white text-black shadow-[0_0_20px_rgba(255,255,255,0.2)] scale-[1.02]";
          else if (isToday) cls = "bg-white/10 text-white border border-white/20";
          return (
            <button key={day} onClick={() => setSelectedDay(i)}
              className={"flex-shrink-0 flex flex-col items-center px-4 py-3 rounded-2xl transition-all duration-200 min-w-[64px] " + cls}>
              <span className="text-[11px] font-bold uppercase tracking-wider">{DAY_SHORT[i]}</span>
              {isToday && <span className={"mt-1 w-1.5 h-1.5 rounded-full " + (isSelected ? "bg-black/40" : "bg-white")} />}
            </button>
          );
        })}
      </div>
      ...
      {dayPlan ? (
        <div className="space-y-4">
          {meals.map(({ key, label, icon: Icon, color, bg, border, timeRange }, mealIdx) => {
            ...
            const cardCls = isActive ? (bg + " " + border + " border shadow-lg") : "glass-card border-transparent";
            return (
              <div key={key} className={"rounded-2xl p-5 border transition-all " + cardCls}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {isActive && (
                    <span className={"px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border " + color + " " + bg + " " + border}>
                      Now open
                    </span>
                  )}
                </div>
                {items.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {items.map((item, i) => (
                      <span key={i} className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white/80 font-medium">{item}</span>
                    ))}
                  </div>
                ) : <p className="text-white/30 text-sm">No items listed</p>}
              </div>
            );
          })}
        </div>
      ) : (
        <div className="glass-card rounded-2xl p-8 text-center text-white/40">No data for {DAYS[selectedDay]}</div>
      )}
    </div>
  );
}
